...
const handleRegister = async () => {
  if (!name.value || !email.value || !emailVerify.value || !password.value || !passwordVerify.value || !documentNumber.value) {
  ...
  if (!validatePassword(password.value)) {
    error.value = 'La contraseña debe tener mínimo 8 caracteres, incluir mayúsculas, minúsculas, números y un caracter especial.'
    return
  }
  loading.value = true
  error.value = ''
  ...
  <div class="min-h-screen bg-gray-100 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
      ...
      <p class="mt-2 text-center text-sm text-gray-600">
        Crea tu cuenta para gestionar tus cursos
      </p>
    </div>
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
      <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
        <form class="space-y-6" @submit.prevent="handleRegister">
          ...
          <div>
            <label for="passwordVerify" class="block text-sm font-medium text-gray-700">Verifica tu contraseña</label>
            <input id="passwordVerify" v-model="passwordVerify" type="password" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
          </div>
          <div>
          ...
          <div>
            ...
            <input id="specialty" v-model="specialty" type="text" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
          </div>
          <div>
